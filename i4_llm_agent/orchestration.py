# [[START MODIFIED orchestration.py - Save Snapshot v0.2.4]]
...
from .session import SessionManager # Assumes v0.2.4 with snapshot methods
...
try:
    ...
    # Define a fallback that matches the *new* expected signature (including context parts)
    async def process_context_and_prepare_payload(*args, **kwargs) -> Tuple[Optional[List[Dict]], Dict[str, Any], Optional[str], Optional[List[Dict]], Optional[str]]:
        ...
        await asyncio.sleep(0)
        # Return structure matching expected output: payload, status, base_prompt, t0_history, combined_context
        return None, {"error": "Context processor unavailable"}, None, None, None
...
# ==============================================================================
# === Session Pipe Orchestrator Class (Save Snapshot Logic v0.2.4)         ===
# ==============================================================================
...
class SessionPipeOrchestrator:
    ...
    Includes logic to skip steps during regeneration.
    Saves context snapshot after normal turns for consistent regeneration.
    Version: 0.2.4
    """
    version = "0.2.4" # <<< Version Updated

    def __init__(
        self,
        config: object,
        session_manager: SessionManager, # Expects v0.2.4+
        ...
        # Check if SessionManager has snapshot methods (basic check)
        if not hasattr(session_manager, 'set_last_context_snapshot') or \
           not hasattr(session_manager, 'get_last_context_snapshot'):
            self.logger.error("CRITICAL: SessionManager instance does not have required snapshot methods. Snapshotting disabled.")
            # Optionally raise an error or disable snapshot feature flag internally
            self._snapshotting_enabled = False
        else:
             self._snapshotting_enabled = True
        ...
        self.logger.info(f"SessionPipeOrchestrator v{self.version} initialized (Save Snapshot Logic).")
        self.logger.info(f"Context Snapshotting Enabled: {self._snapshotting_enabled}")
        # (Other availability checks unchanged)
    ...
    async def _emit_status(self, event_emitter: Optional[Callable], session_id: str, description: str, done: bool = False):
    ...
    async def _async_llm_call_wrapper( self, api_url: str, api_key: str, payload: Dict[str, Any], temperature: float, timeout: int = 90, caller_info: str = "Orchestrator_LLM",) -> Tuple[bool, Union[str, Dict]]:
    ...
    async def _determine_effective_query( self, session_id: str, current_active_history: List[Dict], is_regeneration_heuristic: bool ) -> Tuple[str, List[Dict], Optional[str]]:
    ...
    async def _handle_tier1_summarization( self, session_id: str, user_id: str, current_active_history: List[Dict], is_regeneration_heuristic: bool, event_emitter: Optional[Callable] ) -> Tuple[bool, Optional[str], int, int]:
    ...
    async def _handle_memory_aging( self, session_id: str, user_id: str, event_emitter: Optional[Callable] ) -> bool:
    ...
    async def _handle_tier2_transition( self, session_id: str, t1_success: bool, chroma_embed_wrapper: Optional[Any], event_emitter: Optional[Callable] ) -> None:
    ...
    async def _calculate_and_format_status( self, session_id: str, summarization_prompt_tokens: int, summarization_output_tokens: int, inventory_prompt_tokens: int, final_llm_payload_contents: Optional[List[Dict]], pre_scene_changed_flag: bool, final_confirmed_world_state: Dict[str, Any], final_confirmed_scene_state: Dict[str, Any], final_scene_changed_flag: bool, context_status_info: Dict[str, Any], session_process_owi_rag: bool, aging_performed_flag: bool, inventory_update_success_flag: bool, inventory_update_completed: bool, ) -> Tuple[str, int]:
    ...
    async def _execute_or_prepare_output( self, session_id: str, body: Dict, final_llm_payload_contents: Optional[List[Dict]], event_emitter: Optional[Callable], status_message: str, final_payload_tokens: int ) -> OrchestratorResult:
    ...
    # === MAIN PROCESSING METHOD (Save Snapshot Logic Added v0.2.4) ===
    async def process_turn(
        ...
        and final LLM calls. Skips steps and saves context snapshot if not regenerating.
        """
        ...
        # --- Define Logger Helpers (Unchanged) ---
        debug_path_getter = None
        inventory_log_func = None
        if _UTILS_AVAILABLE:
            debug_path_getter = functools.partial(get_debug_log_path, config=self.config, logger_instance=self.logger)
            async def inventory_logger_wrapper(sid: str, msg: str, log_type: str):
                if getattr(self.config, 'debug_log_final_payload', False):
                    await awaitable_log_inventory_debug(session_id=sid, message=msg, log_type=log_type, config=self.config, logger_instance=self.logger)
        ...
        # <<< Variables to hold context parts for snapshot >>>
        base_prompt_for_snapshot: Optional[str] = None
        t0_history_for_snapshot: Optional[List[Dict]] = None
        combined_context_for_snapshot: Optional[str] = None
        ...
        try:
            ...
            # --- Determine Query, History Slice (Unchanged) ---
            ...
            # --- Fetch Initial World & Scene State (Unchanged) ---
            ...
            # --- Memory Management Sequence (Skip Aging/T2 on regen) (Unchanged) ---
            ...
            if not is_regeneration_heuristic:
                 ...
                 else: aging_performed = await self._handle_memory_aging(session_id, user_id, event_emitter)
            ...
            # --- Generate Hint AND Weather Proposal (Skip if regenerating) (Unchanged) ---
            ...
            if not is_regeneration_heuristic:
                ...
                if event_hints_enabled and self._generate_hint_func:
                    ...
                    if not hint_llm_url or not hint_llm_key: self.logger.warning(f"[{session_id}] Skipping hint: Config incomplete.")
                        ...
                        except Exception as e_hint_gen: self.logger.error(f"[{session_id}] Error during hint generation: {e_hint_gen}", exc_info=True); generated_event_hint_text = None; generated_weather_proposal = {"previous_weather": initial_world_state_dict.get("weather"), "new_weather": None}
                ...
                else: self.logger.debug(f"[{session_id}] Skipping hint: Disabled by global valve.");
            else:
                 self.logger.info(f"[{session_id}] Skipping Hint Generation due to Regeneration flag.")
                 generated_event_hint_text = None

            # --- STAGE 1: Pre-emptive State Assessment (Skip if regenerating) (Unchanged) ---
                ...
                if self._unified_state_func:
                    ...
                    try:
                        ...
                        if not state_assess_llm_config["url"] or not state_assess_llm_config["key"]: self.logger.error(f"[{session_id}] Pre-State LLM URL/Key missing. Skip."); pre_assessed_state_dict = None
                            ...
                            else: self.logger.error(f"[{session_id}] Pre-state assessment invalid data. Using initial for context."); pre_assessed_state_dict = None;
                    except Exception as e_pre_assess: self.logger.error(f"[{session_id}] Exception during pre-state assessment: {e_pre_assess}", exc_info=True); pre_assessed_state_dict = None;
                else: self.logger.error(f"[{session_id}] Skipping pre-state assessment: Function unavailable.");
            else:
                 self.logger.info(f"[{session_id}] Skipping Pre-State Assessment due to Regeneration flag.")
                 pre_assessed_world_state_for_context = initial_world_state_dict.copy()
                 pre_assessed_scene_state_for_context = initial_scene_state_dict.copy()

            # --- Call Context Processor (Expect NEW return signature) ---
            ...
            (final_llm_payload_contents, context_status_info,
             base_prompt_for_snapshot, t0_history_for_snapshot, combined_context_for_snapshot
            ) = await self._context_processor_func(
            ...
            # Check if payload is None, indicates critical failure in processor
            if final_llm_payload_contents is None:
                 raise ValueError("Context processor failed to return payload contents.")
            t1_retrieved_count = context_status_info.get("t1_retrieved_count", 0)

            # --- <<< NEW: Save Context Snapshot if Normal Turn >>> ---
            if not is_regeneration_heuristic and self._snapshotting_enabled:
                if base_prompt_for_snapshot is not None and \
                   t0_history_for_snapshot is not None and \
                   combined_context_for_snapshot is not None:
                    snapshot_data = {
                        "base_prompt": base_prompt_for_snapshot,
                        "t0_history": t0_history_for_snapshot,
                        "combined_context": combined_context_for_snapshot,
                        "latest_user_query": latest_user_query_str, # Also store the query
                        "event_hint": generated_event_hint_text, # And the hint used
                        "timestamp": datetime.now(timezone.utc).isoformat()
                    }
                    save_success = self.session_manager.set_last_context_snapshot(session_id, snapshot_data)
                    if save_success:
                        self.logger.info(f"[{session_id}] Successfully saved context snapshot to session state.")
                    else:
                         self.logger.error(f"[{session_id}] Failed to save context snapshot to session state (check SessionManager).")
                else:
                    self.logger.warning(f"[{session_id}] Skipping snapshot save: Context processor did not return all required snapshot components.")
            elif is_regeneration_heuristic:
                 self.logger.debug(f"[{session_id}] Skipping context snapshot save due to Regeneration flag.")
            elif not self._snapshotting_enabled:
                 self.logger.debug(f"[{session_id}] Skipping context snapshot save as snapshotting is disabled.")
            # --- <<< END NEW SECTION >>> ---


            # --- Execute Final LLM Call or Prepare Output (Unchanged) ---
            ...
            # --- STAGE 2: Post-Turn State Assessment (Finalization - Skip if regenerating) (Unchanged) ---
            ...
            # --- Update Orchestrator State & Save FINAL Confirmed State (Skip save on regen) (Unchanged) ---
            ...
            # --- Post-Turn Inventory Update (Skip if regenerating) (Unchanged) ---
            if not is_regeneration_heuristic:
                            ...
                            if not self.sqlite_cursor or not self.sqlite_cursor.connection: self.logger.error(f"[{session_id}] Cannot update inventory: SQLite cursor invalid."); inventory_update_success_flag = False
                                ...
                                try:
                                    update_success = await self._update_inventories_func(
                                        ...
                                        inventory_log_func=inventory_log_func
                                    )
            ...
            # --- Final Status Calculation and Emission (Unchanged) ---
            ...
            final_status_string, final_payload_tokens = await self._calculate_and_format_status(
                 ...
                 final_confirmed_world_state=final_world_state, final_confirmed_scene_state=final_scene_state,
                 final_scene_changed_flag=final_scene_changed_flag, context_status_info=context_status_info,
                 session_process_owi_rag=bool(getattr(user_valves, 'process_owi_rag', True)),
                 aging_performed_flag=aging_performed, inventory_update_success_flag=inventory_update_success_flag,
...
# [[END MODIFIED orchestration.py - Save Snapshot v0.2.4]]